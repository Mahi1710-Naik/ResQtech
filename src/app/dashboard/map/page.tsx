...
export default function LiveMapPage() {
  const mapBg = PlaceHolderImages.find(img => img.id === 'map-background');

  const getDeviceAvatar = (deviceId: string) => {
    const avatar = PlaceHolderImages.find(img => img.id.includes(deviceId.toLowerCase()) || img.id === (deviceId === 'GB-9921' ? 'ananya-avatar' : 'priya-avatar'));
    return avatar;
  };
      ...
      <div className="flex-1 p-6 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 relative rounded-2xl overflow-hidden bg-slate-200 shadow-inner group">
          ...
          {MOCK_DEVICES.map((device, i) => {
            const avatar = getDeviceAvatar(device.id);
            return (
              <div 
                key={device.id}
          ...
          <div className="absolute bottom-6 right-6 flex flex-col gap-2">
            <Button size="icon" variant="secondary" className="bg-white shadow-lg" suppressHydrationWarning><Navigation className="h-4 w-4" /></Button>
            <Button size="icon" variant="secondary" className="bg-white shadow-lg font-bold text-lg" suppressHydrationWarning>+</Button>
            <Button size="icon" variant="secondary" className="bg-white shadow-lg font-bold text-lg" suppressHydrationWarning>-</Button>
          </div>
